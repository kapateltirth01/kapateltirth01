...
CONFIG_PATH = os.path.join(HERE, "..", "data", "profile-config.json")
...
GITHUB_URL = CONFIG.get("social_links", {}).get("github", "")
...
HOST = github_username(GITHUB_URL)
DISPLAY_NAME = CONFIG.get("display_name", "").strip()
SHORT_BIO = CONFIG.get("short_bio", "").strip()
TAGLINE = CONFIG.get("tagline", "").strip()
CURRENT_STATUS = CONFIG.get("current_status", "").strip()
CURRENT_FOCUS = CONFIG.get("current_focus") or []
PROGRAMMING_LANGUAGES = CONFIG.get("programming_languages") or []
FRAMEWORKS_TOOLS = CONFIG.get("frameworks_tools") or []
OTHER_TECHNICAL_SKILLS = CONFIG.get("other_technical_skills") or []
HIGHLIGHTS = CONFIG.get("highlights") or []

ROWS = [("host",)]
if DISPLAY_NAME:
    ROWS.append(("kv", "Name", DISPLAY_NAME))
if TAGLINE:
    ROWS.append(("kv", "Tagline", TAGLINE))
if SHORT_BIO:
    ROWS.append(("bul", SHORT_BIO))
if CURRENT_STATUS:
    ROWS.append(("kv", "Status", CURRENT_STATUS))

if CURRENT_FOCUS:
    ROWS.append(("gap",))
    ROWS.append(("sec", "Focus"))
    for item in CURRENT_FOCUS:
        ROWS.append(("bul", item))
...
if FRAMEWORKS_TOOLS:
    ROWS.append(("gap",))
    ROWS.append(("sec", "Tools"))
    for item in FRAMEWORKS_TOOLS:
        ROWS.append(("bul", item))

if OTHER_TECHNICAL_SKILLS:
    ROWS.append(("gap",))
    ROWS.append(("sec", "Skills"))
    for item in OTHER_TECHNICAL_SKILLS:
        ROWS.append(("bul", item))

if HIGHLIGHTS:
    ROWS.append(("gap",))
    ROWS.append(("sec", "Highlights"))
