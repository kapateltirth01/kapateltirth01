#!/usr/bin/env python3
"""
Fetch the authenticated user's GitHub contribution calendar via GitHub's
GraphQL API, then write data/contributions.json with the raw daily counts plus
derived stats (current streak, longest streak, best day, monthly totals).

This is the authoritative source for GitHub profile calendar data because it
matches the GitHub profile contribution calendar itself, including the rolling
"last year" date window. It refreshes automatically in the GitHub Action.
"""
import datetime
import json
import os
...
def fetch_days_html_fallback():
    ...
    try:
        ...
        raise RuntimeError(
            "BeautifulSoup is required only for HTML fallback. "
            "Install beautifulsoup4 or provide GH_TOKEN for GraphQL access."
        ) from exc
...
def fetch_days():
    ...
    if days:
        return days
    return fetch_days_html_fallback()
